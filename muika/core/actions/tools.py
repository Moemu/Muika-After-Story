...
class MemoryTool(BaseTool):
    ...
    category: Literal["user", "self", "world"] = Field(
        "user",
        description="Category: 'user' for user facts, 'self' for self-knowledge, 'world' for general facts.",
    )
    ...
    strength: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence / importance (0.0–1.0), used for 'remember'.",
    )
    ...
    async def handle(self, state: "MuikaState", executor: "Executor") -> ActionOutput:
        ...
        if self.type == "read":
            mem = state.memory.memory
            if not mem:
                return ActionOutput(content="No memories stored yet.")
            lines = [
                f"[{v.category}] {v.key}: {v.value} (confidence={v.confidence:.2f})"
                for _, v in sorted(mem.items(), key=lambda x: -x[1].confidence)
        ...
        if self.type == "remember":
            ...
            await state.memory.record_memory(
                type="remember",
            ...
            logger.info(f"[MemoryTool] Saved {self.category}:{self.key} = {self.value!r}")
            return ActionOutput(
                content=f"Memory saved — {self.category}:{self.key} = {self.value!r} (confidence={self.strength})"
            )

        # type == "forget"
        if not self.key:
            return ActionOutput(content="[MemoryTool] 'key' is required for 'forget'.")
        await state.memory.record_memory(type="forget", category=self.category, key=self.key)
        logger.info(f"[MemoryTool] Forgot {self.category}:{self.key}")
        return ActionOutput(content=f"Memory forgotten — {self.category}:{self.key}")
